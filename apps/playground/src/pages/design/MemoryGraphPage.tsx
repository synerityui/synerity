...
const EDGES: { from: NodeId; to: NodeId; type: string }[] = [
  ...
  return (
    <div style={pageShell}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 24 }}>
        <DesignCard padding={0} style={{ overflow: "hidden" }}>
          ...
          <div style={{ position: "relative", background: "var(--syn-bg-sunken)", height: 540 }}>
            <svg width="100%" height="100%" viewBox="0 0 1000 540" preserveAspectRatio="xMidYMid meet">
              ...
              {scored.map((n) => {
                ...
                const r = 10 + n.imp * 18;
                const tagColor = TAG_COLORS[n.tag] ?? "var(--syn-primary)";
                return (
                  <g
                    key={n.id}
                  ...
                  >
                    <circle cx={n.x} cy={n.y} r={r + 6} fill={tagColor} opacity={isIn ? 0.15 : 0} />
                    <circle
                      cx={n.x} cy={n.y} r={r}
                      fill={isIn ? tagColor : "var(--syn-bg-raised)"}
                      stroke={tagColor}
                    ...
                    <text
                      ...
                      fill="var(--syn-text)" textAnchor="middle"
                    >
                      {n.label}
                    </text>
                    <text
        ...
        <div style={{ display: "grid", gap: 16, alignContent: "start" }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input
                ...
                style={{
                  ...
                  fontSize: 13, color: "var(--syn-text)", outline: "none", boxSizing: "border-box",
                }}
              />
            </div>
            <div>
              ...
              <input
                type="range" min={20} max={300} value={budget}
          ...
          <DesignCard padding={0}>
            <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--syn-border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <Mono style={{ fontSize: 11, color: "var(--syn-text-muted)", textTransform: "uppercase", letterSpacing: "0.1em" }}>graph.toContext()</Mono>
              <Badge variant="success">{included.size} nodes</Badge>
            </div>
            <div style={{ padding: 16, fontFamily: "var(--syn-font-mono)", fontSize: 12, lineHeight: 1.6, color: "var(--syn-text)", maxHeight: 280, overflow: "auto" }}>
              ...
              {ranked.filter((n) => included.has(n.id)).map((n) => (
                <div key={n.id}>
                  <span style={{ color: "var(--syn-text-muted)" }}>—</span> {n.label}{" "}
                  <span style={{ color: TAG_COLORS[n.tag] ?? "var(--syn-primary)", fontSize: 10 }}>[{n.tag}]</span>
                </div>
              ))}
            ...
            <div style={{ display: "grid", gap: 6, fontFamily: "var(--syn-font-mono)", fontSize: 12 }}>
              {["relates_to", "contradicts", "depends_on", "follows", "part_of", "causes"].map((t) => (
                <div key={t} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ width: 24, height: 1, background: "var(--syn-border-strong)", display: "inline-block" }} />
                  <span>{t}</span>
                </div>
              ))}
            </div>
      ...
      <div style={{ marginTop: 32 }}>
        ...
        <DesignCard>
          <pre style={{ margin: 0, fontFamily: "var(--syn-font-mono)", fontSize: 13, lineHeight: 1.7, color: "var(--syn-text)", overflowX: "auto" }}>{`import { MemoryGraph } from "@synerity/memory-graph";

const graph = new MemoryGraph();

graph.add("User prefers dark mode", { tag: "preference", importance: 0.9 });
graph.add("Subscription: Pro plan",  { tag: "billing",    importance: 1.0 });
graph.link("n1", "n2", "relates_to");

// Retrieve context under a token budget
const context = graph.toContext({ query: "billing", maxTokens: 120 });`}</pre>
        </DesignCard>
      </div>
    </div>
  );
